...
defmodule AshScylla.DataLayer.MaterializedView do
  ...
  def create_view_cql(view_name, base_table, view_config) do
    ...
    # Generate unqualified identifiers. The keyspace context is applied by the
    # connection (via `USE keyspace`) when statements are executed, keeping the
    # generated DDL portable and consistent with the other migration helpers.
    qualified_table = AshScylla.Identifier.quote_name(base_table)
    qualified_view_name = AshScylla.Identifier.quote_name(to_string(view_name))
