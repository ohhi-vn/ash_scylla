defmodule AshScylla.ScyllaIntegrationTest do
    ...
    case Xandra.start_link(nodes: ["localhost:#{port}"], connect_timeout: 5_000) do
      {:ok, conn} ->
        case Xandra.execute(conn, "SELECT now() FROM system.local") do
          {:ok, _} -> Xandra.stop(conn); true
          {:error, _} -> Xandra.stop(conn); Process.sleep(2_000); if retries > 0, do: scylla_ready?(port, retries - 1), else: false
        end
      {:error, _} -> Process.sleep(2_000); if retries > 0, do: scylla_ready?(port, retries - 1), else: false
    end
  end

  defp docker_run do
    System.cmd("docker", ["run", "-d", "--name", "ash-scylla-test",
      "-p", "#{@cql_port}:#{@cql_port}", @image,
      "--smp", "1", "--memory", "2G", "--developer-mode", "1"], stderr_to_stdout: true)
  end
  ...
  defp find_container do
    case System.cmd("docker", ["ps", "-q", "-f", "name=ash-scylla-test"], stderr_to_stdout: true) do
      {out, 0} -> id = String.trim(out); if id != "", do: {:ok, id}, else: :not_found
      _ -> :not_found
    end
  end

  defp container_running?(id) do
    match?({"true\n", 0}, System.cmd("docker", ["inspect", "-f", "{{.State.Running}}", id], stderr_to_stdout: true))
  end

  defp ensure_container do
    cid = case find_container() do
      {:ok, id} -> if container_running?(id), do: {:ok, id}, else: {:restart, id}
      :not_found -> :new
    end

    case cid do
      {:ok, id} -> {:ok, id, @cql_port}
      {:restart, _} -> docker_stop(); start_and_wait()
      :new -> start_and_wait()
    end
  end
    ...
    case docker_run() do
      {_, 0} ->
        case scylla_ready?(@cql_port) do
          true -> {:ok, @cql_port}
          false -> docker_stop(); case docker_run() do {_, 0} -> scylla_ready?(@cql_port); {:ok, @cql_port}; {err, _} -> {:error, String.trim(err)} end
        end
  ...
  defp xq(conn, query, params \\ []) do
    ...
    result = case Xandra.execute(conn, query, encoded_params) do
      {:ok, page} -> page
  ...
  describe "basic connectivity" do
    ...
    test "keyspace exists", %{conn: c} do
      assert xq(c, "SELECT keyspace_name FROM system_schema.keyspaces WHERE keyspace_name = ?", ["ash_scylla_test"]).num_rows == 1
    end

    test "tables exist", %{conn: c} do
      tables = xq(c, "SELECT table_name FROM system_schema.tables WHERE keyspace_name = ?", ["ash_scylla_test"]).rows |> MapSet.new(&hd/1)
      assert "users" in tables and "events" in tables and "counters" in tables
    end

    test "indexes exist", %{conn: c} do
      names = xq(c, "SELECT index_name FROM system_schema.indexes WHERE keyspace_name = ? AND table_name = ?", ["ash_scylla_test", "users"]).rows |> MapSet.new(&hd/1)
      assert "idx_users_email" in names and "idx_users_status" in names and "idx_users_age" in names
    end
  end
  ...
  describe "CRUD operations" do
    test "insert and select", %{conn: c} do
      id = uid()
      xq(c, "INSERT INTO ash_scylla_test.users (id, name, email, age, status) VALUES (?, ?, ?, ?, ?)",
    ...
    test "insert and select simple", %{conn: c} do
      ...
      [row] = rows_to_maps(xq(c, "SELECT name, age FROM ash_scylla_test.users WHERE id = ?", [id]))
      assert row["name"] == "Bob"
      assert row["age"] == 25
    end
    ...
    test "update specific columns", %{conn: c} do
      id = uid()
      xq(c, "INSERT INTO ash_scylla_test.users (id, name, age) VALUES (?, ?, ?)", [id, "Charlie", 25])
    ...
    test "insert with timestamp fields", %{conn: c} do
      ...
      xq(c, "INSERT INTO ash_scylla_test.users (id, name, created_at) VALUES (?, ?, ?)", [id, "Time User", {:timestamp, now}])
      assert xq(c, "SELECT created_at FROM ash_scylla_test.users WHERE id = ?", [id]).num_rows == 1
    end
  end
  ...
  describe "complex queries" do
    setup %{conn: c} do
      Enum.each(1..20, fn i ->
        ...
        xq(c, "INSERT INTO ash_scylla_test.users (id, name, email, status, age) VALUES (?, ?, ?, ?, ?)",
          [id, "User#{i}", "user#{i}@test.com", status, 20 + i])
      end)
    end

    test "filter by email index", %{conn: c} do
      rows = rows_to_maps(xq(c, "SELECT * FROM ash_scylla_test.users WHERE email = ?", ["user5@test.com"]))
      assert length(rows) >= 1
      assert Enum.any?(rows, fn row -> row["name"] == "User5" end)
    end

    test "filter by status index", %{conn: c} do
      rows = rows_to_maps(xq(c, "SELECT * FROM ash_scylla_test.users WHERE status = ?", ["active"]))
    ...
    test "filter by status with LIMIT", %{conn: c} do
      rows = rows_to_maps(xq(c, "SELECT * FROM ash_scylla_test.users WHERE status = ? LIMIT 5", ["active"]))
    ...
    test "IN clause", %{conn: c} do
      ...
      Enum.each(ids, &xq(c, "INSERT INTO ash_scylla_test.users (id, name) VALUES (?, ?)", [&1, "IN Test"]))
    ...
    test "clustering key range", %{conn: c} do
      user_id = uid()
      Enum.each(1..5, fn i ->
        xq(c, "INSERT INTO ash_scylla_test.events (user_id, event_type, event_id, payload) VALUES (?, ?, now(), ?)", [user_id, "click", "e#{i}"])
      end)
      assert xq(c, "SELECT * FROM ash_scylla_test.events WHERE user_id = ? AND event_type = ? LIMIT 3", [user_id, "click"]).num_rows == 3
    end
    ...
    test "materialized view", %{conn: c} do
      ...
      Process.sleep(1000)
      rows = rows_to_maps(xq(c, "SELECT * FROM ash_scylla_test.users_by_email WHERE email = ?", ["mv@test.com"]))
      assert length(rows) >= 1
  ...
  describe "TTL support" do
    test "insert with short TTL and verify expiry", %{conn: c} do
      id = uid()
      xq(c, "INSERT INTO ash_scylla_test.users (id, name) VALUES (?, ?) USING TTL 2", [id, "Short Lived"])
    ...
    test "insert with long TTL persists", %{conn: c} do
      id = uid()
      xq(c, "INSERT INTO ash_scylla_test.users (id, name) VALUES (?, ?) USING TTL 3600", [id, "Long Lived"])
      assert xq(c, "SELECT * FROM ash_scylla_test.users WHERE id = ?", [id]).num_rows == 1
    end
  end
  ...
  describe "counter operations" do
    test "increment and decrement counters", %{conn: c} do
      id = uid()
      xq(c, "UPDATE ash_scylla_test.counters SET views = views + 1, likes = likes + 1 WHERE id = ?", [id])
  ...
  describe "concurrent read/write simulation" do
    ...
    test "concurrent reads on same secondary index query" do
      ...
      id = uid()
      xq(setup_conn, "INSERT INTO ash_scylla_test.users (id, name, email, status) VALUES (?, ?, ?, ?)",
      ...
      tasks = Enum.map(1..10, fn _ ->
        Task.async(fn ->
          {:ok, conn} = Xandra.start_link(nodes: ["localhost:#{port}"])
          {:ok, result} = Xandra.execute(conn, "SELECT * FROM ash_scylla_test.users WHERE email = ?", [encode_param("shared@test.com")])
          Xandra.stop(conn)
    ...
    test "concurrent event writes to same partition" do
      ...
      tasks = Enum.map(1..20, fn i ->
        Task.async(fn ->
          {:ok, conn} = Xandra.start_link(nodes: ["localhost:#{port}"])
          xq(conn, "INSERT INTO ash_scylla_test.events (user_id, event_type, event_id, payload) VALUES (?, ?, now(), ?)",
    ...
    test "mixed CRUD operations under concurrent load" do
      ...
      Enum.each(base_ids, &xq(setup_conn, "INSERT INTO ash_scylla_test.users (id, name, status, age) VALUES (?, ?, ?, ?)", [&1, "CRUD", "active", 25]))
      Xandra.stop(setup_conn)

      tasks = Enum.flat_map(base_ids, fn id ->
        [Task.async(fn ->
          {:ok, conn} = Xandra.start_link(nodes: ["localhost:#{port}"])
          result = Xandra.execute(conn, "SELECT * FROM ash_scylla_test.users WHERE id = ?", [encode_param(id)])
          Xandra.stop(conn)
          result
        end),
        Task.async(fn ->
          {:ok, conn} = Xandra.start_link(nodes: ["localhost:#{port}"])
          result = Xandra.execute(conn, "UPDATE ash_scylla_test.users SET age = ? WHERE id = ?", [encode_param(30), encode_param(id)])
          Xandra.stop(conn)
          result
        end)]
      end)
  ...
  describe "DataLayer query struct against real DB" do
    test "build_optimized_query produces valid CQL", %{conn: c} do
      ...
      query = %DataLayer{
        resource: nil, repo: TestRepo, table: "ash_scylla_test.users",
        filters: [
          %{operator: :eq, left: %{name: :status}, right: %{value: "active"}}
        ],
        sorts: [], limit: 10, offset: nil,
        select: [:id, :name, :email], tenant: nil
      }
    ...
    test "build_optimized_query with IN operator", %{conn: c} do
      ...
      query = %DataLayer{
        ...
        sorts: [], limit: nil, offset: nil, select: nil, tenant: nil
      }
  ...
  describe "filter validation against real schema" do
    test "validates filters on indexed columns pass" do
      assert :ok == AshScylla.DataLayer.FilterValidator.validate_filters(
