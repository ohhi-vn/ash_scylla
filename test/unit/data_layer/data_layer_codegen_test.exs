...
defmodule AshScylla.ExtensionTest do
  ...
  describe "generated migration file content" do
    test "uses heredoc triple-quotes for @moduledoc (not invalid double-quotes)" do
      # Verifies the fix for the invalid @moduledoc syntax bug where
      # the generator produced @moduledoc "" instead of @moduledoc """
      # which caused SyntaxError in generated migration files.

      resource = AshScylla.TestResource
      statements = ["CREATE TABLE IF NOT EXISTS test_table (id UUID PRIMARY KEY, name TEXT)"]

      content = generate_test_migration_content(resource, statements)

      # The @moduledoc must use heredoc triple-quotes
      assert content =~ ~r/@moduledoc """\n/,
             "Expected @moduledoc to use heredoc triple-quotes, got:\n#{content}"

      # Must NOT contain the broken pattern
      refute content =~ ~r/@moduledoc ""\n\s*Migration/,
             "Found invalid @moduledoc \"\" pattern:\n#{content}"

      # The closing triple-quote must be present
      assert content =~ ~r/\n\s*"""/,
             "Expected closing triple-quote for heredoc, got:\n#{content}"

      # Verify the content includes the expected text
      assert content =~ "Migration for"
      assert content =~ "mix ash.codegen"

      # Verify the generated content is valid Elixir syntax by parsing it
